class Admin::PrefecturesController < AdminController
  ...
  def create
    ...
    respond_to do |format|
      if @prefecture.save
        format.html { redirect_to [:admin, @prefecture], notice: 'Prefecture was successfully created.' }
        format.json { render json: [:admin, @prefecture], status: :created, location: @prefecture }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_prefectures_url }
      format.json { head :no_content }
    end
  end
end
